...
public class ChatMessageServiceImpl implements ChatMessageService {
    ...
    public Map<String, List<ChatMessage>> getChatMessagesByDate(String uid, String friendUid) {
        ...
        Map<String , List<ChatMessage>> map = new LinkedHashMap<>();        // 순서 보장
        for (ChatMessage cm: mergedList) {
            String date = cm.getTimestamp().toString().substring(0,10);     // 날짜만 (키 값)
            if (map.containsKey(date)) {
                List<ChatMessage> cmList = map.get(date);       // value 값
                cmList.add(cm);
